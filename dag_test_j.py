...
with DAG('dag_test_j',
    ...
    def gen(**kwargs):
        ti = kwargs['ti']
        ids = ti.xcom_pull(task_ids='push_param_task', key='site_ids')
        xx = []
        for id in ids.split(","):
            xx.append({"bash_command": "echo " + id})

        bash_echo = BashOperator.partial(task_id="bash").expand_kwargs(xx)

        previous_task = my
        previous_task >> bash_echo
